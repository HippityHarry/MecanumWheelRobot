...
void sendFrameOverWebSocket() {
  // No HTTP handling, directly handle WebSocket communication
  camera_fb_t * fb = esp_camera_fb_get();  // Capture frame from the camera
  if (!fb) {
      Serial.println("Camera capture failed");
      return;  // Return early if capture fails
  }

  // Send frame over WebSocket
  webSocket.broadcastBIN(fb->buf, fb->len); // Broadcast the frame to all connected clients
...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t * payload, size_t length) {
  // We can send custom commands from the controller to the camera over WebSocket here
}
...
void setup() {
    ...
    config.frame_size = FRAMESIZE_QVGA; // Or FRAMESIZE_CIF
...
void loop() {
   sendFrameOverWebSocket();  // Handle video stream over WebSocket
    webSocket.loop();
}
